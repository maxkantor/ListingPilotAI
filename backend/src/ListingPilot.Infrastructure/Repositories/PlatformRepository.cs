...
{
    Task<List<ListingProject>> GetListingsAsync();
    Task<List<GeneratedAsset>> GetAssetsByListingIdAsync(string listingId);
    Task<WorkspaceSettings> GetSettingsAsync();
    Task<WorkspaceSettings> SaveSettingsAsync(WorkspaceSettings settings);
    ...
    Task<List<DemoRequest>> GetDemoRequestsAsync();
    Task<ContactSubmission> SaveContactSubmissionAsync(ContactSubmission submission);
    Task<DemoRequest> SaveDemoRequestAsync(DemoRequest request);
}

public class InMemoryPlatformRepository : IPlatformRepository
{
    private static readonly List<ListingProject> Listings =
    [
        new()
        {
            Id = "listing-101",
    ...
    private static readonly List<GeneratedAsset> Assets =
    [
        new()
        {
            Id = "asset-mls-101",
    ...
    private static WorkspaceSettings Settings = new()
    {
        Id = "settings-default",
        DefaultTone = "Luxury",
    ...
    private static readonly List<ContactSubmission> ContactSubmissions = [];
    private static readonly List<DemoRequest> DemoRequests = [];

    public Task<List<ListingProject>> GetListingsAsync() => Task.FromResult(Listings.OrderByDescending(x => x.UpdatedAt).ToList());

    public Task<List<GeneratedAsset>> GetAssetsByListingIdAsync(string listingId) =>
        Task.FromResult(Assets.Where(x => x.ListingId == listingId).OrderByDescending(x => x.UpdatedAt).ToList());

    public Task<WorkspaceSettings> GetSettingsAsync() => Task.FromResult(Settings);

    public Task<WorkspaceSettings> SaveSettingsAsync(WorkspaceSettings settings)
    {
        Settings = settings;
        return Task.FromResult(Settings);
    }

    public Task<List<UserAccount>> GetUsersAsync() => Task.FromResult(Users.OrderByDescending(x => x.LastActiveAt).ToList());

    public Task<List<Lead>> GetLeadsAsync() => Task.FromResult(Leads.ToList());

    public Task<List<SubscriptionPlan>> GetPlansAsync() => Task.FromResult(Plans.ToList());

    public Task<List<AuditEvent>> GetAuditEventsAsync() => Task.FromResult(AuditEvents.OrderByDescending(x => x.CreatedAt).ToList());

    public Task<List<ContactSubmission>> GetContactSubmissionsAsync() => Task.FromResult(ContactSubmissions.OrderByDescending(x => x.CreatedAt).ToList());

    public Task<List<DemoRequest>> GetDemoRequestsAsync() => Task.FromResult(DemoRequests.OrderByDescending(x => x.CreatedAt).ToList());

    public Task<ContactSubmission> SaveContactSubmissionAsync(ContactSubmission submission)
    {
        ContactSubmissions.Add(submission);
        return Task.FromResult(submission);
    }

    public Task<DemoRequest> SaveDemoRequestAsync(DemoRequest request)
    {
        DemoRequests.Add(request);
